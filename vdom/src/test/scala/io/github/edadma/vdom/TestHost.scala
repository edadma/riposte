...
final class TestListener(val event: String, val capture: Boolean, val fn: Any => Unit)
...
final class TestHost extends HostConfig:
  ...
  def addListener(node: AnyRef, event: String, capture: Boolean, once: Boolean, passive: Boolean, fn: Any => Unit): AnyRef =
    val l = new TestListener(event, capture, fn)
  ...
  /** Invoke the bubble-phase listeners registered for `event` on `node`. */
