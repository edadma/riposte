...
// tree (which mirrors the real host tree) and mutates the host to match.
//
//   VText     — a text node
//   VElement  — a host element with props and children
...
//
// Host nodes appear here only as opaque `AnyRef`s (a portal target, a ref's live
// node) and event handlers as the erased `Any => Unit`; the host library re-adds
// concrete typing in its own DSL. Nothing in this file names a platform type, so
// the model compiles unchanged on the browser, a native toolkit, and the JVM.
sealed trait VNode
...
// children. (Element children go through the host DSL's own String/Int
// conversions; an implicit conversion is never chained, so these don't collide.)
object VNode:
...
// Renders `child` into a different host container (`target`) while leaving a
...
// nodes live under `target`. For overlays, modals, and tooltips that must escape an
// ancestor's clipping or stacking context. `target` is an opaque host node; the
// host DSL narrows it to its real element type. Built via `portal(target, child)`.
final case class VPortal(target: AnyRef, child: VNode) extends VNode
...
// reaches the host: most attributes go through setAttribute, a few well-known
// names (value, checked) are set as live properties, handlers become event
// listeners, styles are written onto the element's style, and RawHtml is assigned
// to innerHTML.
sealed trait Prop

final case class Attr(value: String)      extends Prop
final case class BoolAttr(value: Boolean) extends Prop
// The handler is erased to `Any => Unit`: the host delivers whatever event object
// it produces, and the DSL that built this stored a typed handler cast to this
// shape. `options` carries the addEventListener flags.
...
// Inner markup set verbatim from a trusted string (via `unsafeHtml`). Written to
// the element's innerHTML, replacing its content. Trusted input only.
final case class RawHtml(html: String) extends Prop

// Binds a VElement to its live host node. `attach` runs once the element is
...
sealed trait ElementRef:
  private[vdom] def attach(node: AnyRef): Unit
...
final case class BoxRef[T](box: Ref[T]) extends ElementRef:
  def attach(node: AnyRef): Unit = box.current = node.asInstanceOf[T]
...
final case class FnRef(fn: (AnyRef | Null) => Unit) extends ElementRef:
  def attach(node: AnyRef): Unit = fn(node)
