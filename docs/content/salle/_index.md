...
- **[Display](/salle/display/)** — `ImageCard`, `Badge`/`Tag`, `Skeleton`, and the
  `Spinner`/`Progress` busy indicators.
- **[Navigation](/salle/navigation/)** — `Pagination` and `Dropdown`.
- **[Overlays](/salle/overlays/)** — `Modal` and `Toast`.
